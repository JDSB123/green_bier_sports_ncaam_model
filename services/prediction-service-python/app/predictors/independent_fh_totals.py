...
@dataclass
class FHTotalFactors:
    base_calibration: float = 0.75
...
class IndependentFHTotalModel(BasePredictor):
    MODEL_NAME = "IndependentFHTotal"
    MODEL_VERSION = "1.0.0"
    MARKET_TYPE = "total"
    CALIBRATION = 3.5
    HCA = 0.0
    ...
    def _calculate_variance(self, home: TeamRatings, away: TeamRatings) -> float:
        ...
        tempo_diff = abs(home.tempo - away.tempo) / 2
        variance += tempo_diff * 0.05
        home_3pr = getattr(home, 'three_pt_rate', 35.0) or 35.0
        away_3pr = getattr(away, 'three_pt_rate', 35.0) or 35.0
        avg_3pr = (home_3pr + away_3pr) / 2
        if avg_3pr > 35.0:
            variance += (avg_3pr - 35.0) * 0.05
        return variance
    ...
    def predict(self, home: TeamRatings, away: TeamRatings, is_neutral: bool = False, home_rest_days: Optional[int] = None, away_rest_days: Optional[int] = None) -> MarketPrediction:
        ...
        if home_rest_days is not None and away_rest_days is not None:
            situational_adj = self.calculate_situational_adjustment(home_rest_days, away_rest_days) * 0.5
            situational_adj *= 0.15
        ...
        if prelim_total < 65 or prelim_total > 77.5:
            base_confidence = 0.55
        adj_penalty = min(abs(adjustment) * 0.01, 0.075)
...
independent_fh_total_model = IndependentFHTotalModel()
