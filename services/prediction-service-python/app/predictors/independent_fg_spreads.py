...
class FGSpreadFactors:
    ...
    tempo_adj_factor: float = 0.5
    rest_diff_threshold: int = 3
    rest_adj_per_day: float = 1.0


class IndependentFGSpreadModel(BasePredictor):
    MODEL_NAME = "IndependentFGSpread"
    MODEL_VERSION = "1.0.0"
    MARKET_TYPE = "spread"
    CALIBRATION = 0.0
    MIN_EDGE = 1.5
    ...
    def predict(self, home: TeamRatings, away: TeamRatings, is_neutral: bool = False, home_rest_days: Optional[int] = None, away_rest_days: Optional[int] = None) -> MarketPrediction:
        ...
        confidence = 0.7 - (abs(adjustment) * 0.05)
...
independent_fg_spread_model = IndependentFGSpreadModel()
