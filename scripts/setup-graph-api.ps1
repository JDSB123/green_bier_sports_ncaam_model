...
Write-Host "az containerapp secret set -n ncaam-stable-prediction -g NCAAM-GBSV-MODEL-RG --secrets graph-client-id=$appId graph-client-secret=$clientSecret graph-tenant-id=$tenantId"
Write-Host "az containerapp update -n ncaam-stable-prediction -g NCAAM-GBSV-MODEL-RG --set-env-vars GRAPH_CLIENT_ID=secretref:graph-client-id GRAPH_CLIENT_SECRET=secretref:graph-client-secret GRAPH_TENANT_ID=secretref:graph-tenant-id"
