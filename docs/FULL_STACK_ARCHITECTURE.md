# NCAAM v33.0 Model FINAL - Complete Self-Contained Stack
...
└─────────────────────────────────────────────────────────────────────────────┘
                    ...
                    ▼               ▼               ▼
        ┌─────────────────┐ ┌──────────────┐ ┌──────────────────┐
        │   PostgreSQL    │ │    Redis     │ │ Prediction       │
        │  (TimescaleDB)  │ │   (Cache)    │ │   Service        │
        │                 │ │              │ │                  │
        │ Port: 5450      │ │ Port: 6390   │ │ Port: 8092      │
        │                 │ │              │ │                  │
        │ Secrets:        │ │ Secrets:     │ │ Secrets:         │
        │ - db_password   │ │ - redis_pwd  │ │ - db_password    │
        │                 │ │              │ │ - redis_password │
        │                 │ │              │ │ - odds_api_key    │
        └─────────────────┘ └──────────────┘ └──────────────────┘
                │                   │                   │
...
- **Container:** `ncaam_v33_model_postgres`
- **Image:** `timescale/timescaledb:latest-pg15`
- **Port:** `5450:5432`
- **Volume:** `ncaam_v33_model_postgres_data` (persistent)
- **Secrets:** `db_password` → `/run/secrets/db_password`
- **Migrations:** Auto-runs from `./database/migrations/` on first init
- **Network:** `ncaam_v33_model_data`

**What's Inside:**
- ✅ **447 teams** (canonical names)
- ✅ **861 team aliases** (from migrations + runtime)
- ✅ **365 team ratings** (from Barttorvik)
- ✅ **Validation functions** (`validate_team_name`, `validate_game_teams`)
- ✅ **Audit table** (`team_resolution_audit`)

### 2. **Redis**
- **Container:** `ncaam_v33_model_redis`
...
- **Network:** `ncaam_v33_model_backend`, `ncaam_v33_model_data`

**What's Inside:**
- ✅ **Python 3.12** (prediction engine)
- ✅ **Go binary** (`/app/bin/ratings-sync`) - Barttorvik sync
- ✅ **Rust binary** (`/app/bin/odds-ingestion`) - Odds API sync
- ✅ **All dependencies** locked in `requirements.txt`

---

## 🔐 **Secrets Management - NO FALLBACKS**

### **Required Secret Files:**
```
secrets/
├── db_password.txt          (REQUIRED - auto-generated by ensure_secrets.py)
├── redis_password.txt       (REQUIRED - auto-generated by ensure_secrets.py)
└── odds_api_key.txt         (REQUIRED - must be created manually)
```
...
### **Code Behavior:**
- ✅ **Rust:** `read_secret_file()` - fails if file doesn't exist
...
Go Binary (ratings-sync)
    ↓
1. resolve_team_name(team_name) → canonical_name
...
Rust Binary (odds-ingestion)
    ↓
1. resolve_team_name(home_team) → home_team_id
2. resolve_team_name(away_team) → away_team_id
3. Validate: home_team_id ≠ away_team_id
...
## 🎯 **Team Name Matching - 100% Accuracy**
...
2. Find: alias="Wisconsin Badgers" → team_id
...
- ✅ Validates `home_team_id ≠ away_team_id` before creating game
- ✅ Logs all resolutions to audit table
- ✅ Fails hard if validation fails

---

## 🚫 **What's NOT Used (Removed/Disabled)**

- ❌ `.env` files - **REMOVED**
- ❌ `godotenv.Load()` - **REMOVED** (Go)
- ❌ `dotenvy::dotenv()` - **REMOVED** (Rust)
- ❌ `load_dotenv()` - **REMOVED** (Python)
- ❌ `localhost:5432` fallbacks - **REMOVED**
- ❌ `localhost:6379` fallbacks - **REMOVED**
...
- ✅ Python 3.12 + all packages from `requirements.txt`
- ✅ Go binary (`ratings-sync`) - compiled inside container
- ✅ Rust binary (`odds-ingestion`) - compiled inside container
- ✅ All source code copied into `/app/`
- ✅ All secrets mounted at `/run/secrets/*`

### **PostgreSQL Container:**
- ✅ All migrations in `/docker-entrypoint-initdb.d/`
- ✅ Auto-runs on first init
- ✅ Persistent data in Docker volume

### **Redis Container:**
- ✅ Password-protected (from secret)
...
- ✅ All secrets from Docker secret files
- ✅ All code compiled inside containers
