...
$env:ODDS_API_KEY = "your_actual_key_here"

# Backward-compatible (still accepted, but ODDS_API_KEY is preferred)
...
secrets/odds_api_key.txt → /run/secrets/odds_api_key
```
...
az containerapp update \
  ...
  --set-env-vars ODDS_API_KEY="your_actual_key"
```
...
|--------|------|----------|----------|
| `ODDS_API_KEY` | API Key | ✅ YES | [Get from here](https://the-odds-api.com/) |
| `THE_ODDS_API_KEY` | API Key | ✅ Compat | Legacy name (still accepted) |
| `DB_PASSWORD` | Database | ✅ YES | Generated by `ensure_secrets.py` |
| `REDIS_PASSWORD` | Cache | ✅ YES | Generated by `ensure_secrets.py` |
| `BASKETBALL_API_KEY` | API Key | ❌ Optional | [Get from here](https://api-basketball.com/) |
| `TEAMS_WEBHOOK_SECRET` | Webhook | ❌ Optional | Teams Outgoing Webhook (validates Teams → API requests) |
...
**Example:** If you set `ODDS_API_KEY` environment variable, it will be used everywhere (local, Docker, Azure) even if the local secrets file exists.
...
az containerapp create \
  ...
  --env-vars ODDS_API_KEY=@odds_api_key
```
...
dir env: | Where-Object Name -Match "^(DB_|REDIS_|ODDS|THE_ODDS)" | Select-Object Name, Value
```
...
### Error: "MISSING REQUIRED SECRET: ODDS_API_KEY"
...
$env:ODDS_API_KEY = "your_key_from_https://the-odds-api.com"
...
- Or set env var: `$env:ODDS_API_KEY = "your_key"`
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run tests
        env:
          ODDS_API_KEY: ${{ secrets.ODDS_API_KEY }}  # from GitHub Secrets
