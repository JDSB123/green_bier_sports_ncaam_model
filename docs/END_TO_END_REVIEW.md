...
- ✅ **Zero external dependencies** (no .env files, no localhost fallbacks)
- ✅ **Docker secrets only** - all secrets from `/run/secrets/*` files
- ✅ **Manual-only operation** - no GitHub Actions, no automatic triggers
- ✅ **Complete data pipeline** - ratings sync → odds ingestion → predictions → executive table
- ✅ **6 markets per game** - Full game & First half (Spread, Total, ML)
- ✅ **Team matching accuracy** - 688+ aliases, 99.99% resolution rate
...
│  │    (Go)      │  │    (Rust)     │  │    (Python)      │ │
│  │              │  │               │  │                  │ │
│  │ • Fetches    │  │ • Fetches     │  │ • Barttorvik     │ │
│  │   Barttorvik │  │   The Odds    │  │   efficiency     │ │
│  │   ratings    │  │   API         │  │   model          │ │
│  │ • 365 teams  │  │ • 72 events   │  │ • 6 markets      │ │
│  │ • Daily sync │  │ • 30s poll    │  │ • Edge calc      │ │
│  └──────┬───────┘  └──────┬────────┘  └────────┬─────────┘ │
│         │                 │                     │           │
│         └─────────────────┴─────────────────────┘           │
│                          │                                  │
...
3. **Container Startup:** Docker Compose starts postgres → redis → prediction-service
4. **Data Sync:**
...
5. **Prediction Engine:**
   ...
   - Calculates 6 markets per game (Full/1H × Spread/Total/ML)
...
### ✅ **FULLY SELF-CONTAINED - NO FALLBACKS**

**Secret Files (Required):**
```
secrets/
├── db_password.txt          # Auto-generated by ensure_secrets.py
├── redis_password.txt       # Auto-generated by ensure_secrets.py
└── odds_api_key.txt         # MUST be created manually
```
...
# ✅ db_password, redis_password, odds_api_key all present
```
...
2. **`team_aliases`** - 688+ name variants
   - Maps "Odds API" → "Barttorvik" names
...
- **`validate_game_teams(game_id)`** - Ensures home ≠ away
...
| `HCA_TOTAL` | 4.5 pts | Home court advantage for totals (×0.2 = 0.9 pts) |
| `HCA_SPREAD_1H` | 1.5 pts | First half spread HCA (50% of full) |
| `HCA_TOTAL_1H` | 2.25 pts | First half total HCA (×0.1 = 0.225 pts) |
...
3. **Full Game Moneyline** - Win probability → American odds
4. **First Half Spread** - 1H away team spread
5. **First Half Total** - 1H Over/Under
6. **First Half Moneyline** - 1H win probability → odds
...
9. **Fire** - Rating 1-5 (◆◆◆◆◆ = 5 = MAX)

**Example:**
```
┃ 21:00      │ Florida @ Saint Mary's              │ FULL   │ ML       │ Florida ML (+742)         │ 48%        │ See odds        │ 40.6%    │ ◆◆◆◆◆  ┃
```

**Fire Rating Scale:**
- **5 (◆◆◆◆◆)** - Edge > 15 pts or > 30% (ML)
- **4 (◆◆◆◆◇)** - Edge 10-15 pts or 20-30% (ML)
- **3 (◆◆◆◇◇)** - Edge 7-10 pts or 15-20% (ML)
- **2 (◆◆◇◇◇)** - Edge 4-7 pts or 10-15% (ML)
- **1 (◆◇◇◇◇)** - Edge 2.5-4 pts or 5-10% (ML)
